...
console.log(token);
let msgReactId = "";
const minimalTime = 30;
let playerPresence; // <tag, timestamp>
let total; // <tag, totalPresence>
let status = false; // true if the bot is currently looking
let cmdList = ["?help", "?status", "?start", "?stop", "?total", "?clear", "?export", "?inscription", "?adminInscription", "?sendRapport"]; // list of commands
let txtChannel = "";
...
let saveTotal; // <tag, totalPresence>
let dataValue;
                ...
                if (members.size > 0) {
                    for (let member of members.values()) {
                        if (playerPresence.has(member.user.tag)) {
                            let v = new Date().getTime() - playerPresence.get(member.user.tag);
...
client.on('messageCreate', async (message) => {
    if (message.content.startsWith('?sendRapport') && message.channel.id === rapportChannel) {
        let debugChannel = message.guild?.channels.cache.find((channel) => channel.id === logRapportChannel);
...
function sendPrivateChannelMessage(message, msg) {
